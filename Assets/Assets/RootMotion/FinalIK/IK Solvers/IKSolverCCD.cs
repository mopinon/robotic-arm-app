                ...
                for (int i = bones.Length - 2; i > -1; i--)
                {
                    ...
                    if (w > 0f)
                    {
                        ...
                        bones[i].transform.rotation =
                            Quaternion.AngleAxis(Mathf.DeltaAngle(angleToLastBone, angleToTarget) * w, Vector3.back) *
                            bones[i].transform.rotation;
                    }
                ...
                for (int i = bones.Length - 2; i > -1; i--)
                {
                    ...
                    if (w > 0f)
                    {
                        ...
                        Quaternion targetRotation =
                            Quaternion.FromToRotation(toLastBone, toTarget) * bones[i].transform.rotation;

                        if (w >= 1) bones[i].transform.rotation = targetRotation;
                        else
                            bones[i].transform.rotation =
                                Quaternion.Lerp(bones[i].transform.rotation, targetRotation, w);
                        //Debug.Log($"Before limits for {i}: {targetRotation.eulerAngles}");
                    }

                    // Rotation Constraints
                    if (useRotationLimits && bones[i].rotationLimit != null) bones[i].rotationLimit.Apply();
                    //Debug.Log($"After limits: X: {bones[i].transform.rotation.x} Y: {bones[i].transform.rotation.y} Z: {bones[i].transform.rotation.z}");
                }
            }
        }
    }
